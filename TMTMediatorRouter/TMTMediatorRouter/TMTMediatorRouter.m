...
#import "TMTMediatorRouter.h"
@interface TMTMediatorRouter()
/**
参数配置
 */
@property (nonatomic, strong, readwrite) TMTMediatorConfig *config;
/**
 APP的控制器映射表
 */
@property (nonatomic, strong) TMTRouteMapModel * URLModuleMap;
//target缓存列表
@property (nonatomic, strong) NSMutableDictionary *targetCache;
@end

@implementation TMTMediatorRouter
#pragma mark - static methods
+ (void)setupConfig:(void(^)(TMTMediatorConfig * _Nonnull))config {
...
+ (instancetype)shared {
     static TMTMediatorRouter *mediator;
     static dispatch_once_t onceToken;
     dispatch_once(&onceToken, ^{
          mediator = [[TMTMediatorRouter alloc] init];
     });
     return mediator;
}
...
- (TMTRouteMapModel *)URLModuleMap {
     if (_URLModuleMap == nil) {
          _URLModuleMap = [TMTRouteMapModel mapWithPlistFile:self.config.URLRouteMapFilePath];
     }
...
{
     //获取URL解析后的模型
     TMTSplitURLModel *obj = [TMTSplitURLModel splitObjectWithURL:url];
